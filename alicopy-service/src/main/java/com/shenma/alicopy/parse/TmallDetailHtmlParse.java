...
import com.shenma.alicopy.ecxeption.CopyBussinessEnums;
...
@Component
public class TmallDetailHtmlParse  extends BaseDetailHtmlParse implements DetailHtmlParse {

	protected static Logger logger = Logger.getLogger("TmallDetailHtmlParse");
	...
	public DetailHtmlParseBean parse(String url) throws CopyBussinessException {
		...
		try {
			...
			offer.setSubject(parseSubject(skuInfo));//标题
			...
			parsePrice(skuInfo, offer);  //设置价格类型和价格
	 ...
	 */
	public String  parseSubject(Map<String,Object> skuInfo) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		String subject=BeanUtils.getProperty(skuInfo, "itemDO.title");
		return subject;
	}
	 ...
	 * 
	 */
	public void parsePrice(Map<String,Object> skuInfo,DetailHtmlParseBean offer) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		if(offer.getSkuProps().size()>0){
			offer.setPriceType(2);
		}else{
			offer.setPriceType(1);
		}
		String price=BeanUtils.getProperty(skuInfo, "detail.defaultItemPrice");
	...
	private void parseSkuList(Document doc,DetailHtmlParseBean offer,Map<String,Object> skuInfo) throws JsonParseException, JsonMappingException, IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		Elements elements=doc.select("#J_DetailMeta .tb-key ul[data-property]");
		for(Element ele:elements){
	...
	public boolean validateUrl(String url) {
		...
		if(url.indexOf("tmall.com")>-1)return true;
		return false;
	}
