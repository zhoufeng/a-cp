...
public class AlibabafinalSaveService {
	...
	public Offer save(String url, boolean isSavaPic, MqRecordItem mqItem,Map<String,Object> selfInfo)
			throws IOException, AliReqException, InterruptedException, IllegalAccessException, InvocationTargetException {
	...
	private void setCatProperties() throws JsonParseException,
			JsonMappingException, IOException, AliReqException, IllegalAccessException, InvocationTargetException {
		...
		if (childrenFids.size() > 0) {
			for (Map<String, Object> featureIdValue : featureIdValues) {
				if (featureIdValue.get("value").toString().equals(value)) {// 看是否有子项
					//vid在之前都是在featureIdValue这个属性里,现在新的版本在childrenFids
					String vid = null;
					if(!featureIdValue.containsKey("vid")){
						vid =childrenFids.get(0).toString();

					}else{
						vid = featureIdValue.get("vid").toString();
					}
